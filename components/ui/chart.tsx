...
export function ChartContainer({ config, children, className }: ChartContainerProps) {
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        className={className}
        style={{ "--chart-1": "217.2 91.2% 59.8%", "--chart-2": "262.1 83.3% 57.8%" } as React.CSSProperties}
      >
        {children}
      </div>
    </ChartContext.Provider>
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-2 shadow-sm dark:border-slate-800 dark:bg-slate-950">
      <div className="grid grid-cols-2 gap-2">
        <div className="flex flex-col">
          <span className="text-[0.70rem] uppercase text-slate-500">Time</span>
          <span className="font-bold text-slate-900 dark:text-slate-50">{label}</span>
        </div>
        {payload.map((item) => {
          ...
          return (
            <div key={key} className="flex flex-col">
              <span className="text-[0.70rem] uppercase text-slate-500">{name}</span>
...
export const ChartTooltip = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<"div"> & ChartTooltipProps>(
  ({ className, ...props }, ref) => {
